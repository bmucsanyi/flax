...
def vmap(
  ...
  *,
  ...
  in_axes_with_vmap_state = jax.tree.map(
    lambda x: VmapStates(x.axes) if isinstance(x, StateAxes) else x,
    in_axes,
  )
  out_axes_with_vmap_state = jax.tree.map(
    lambda x: VmapStates(x.axes) if isinstance(x, StateAxes) else x,
    out_axes,
  )
  ...
  vmapped_fn = jax.vmap(
    vmap_fn,
    in_axes=(
      in_axes_with_vmap_state,  # args
      None,  # vmap_inputs
    ),
    out_axes=(
      in_axes_with_vmap_state,  # args_out
      out_axes_with_vmap_state,  # out_axes
    ),
    axis_name=axis_name,
    axis_size=axis_size,
    spmd_axis_name=spmd_axis_name,
